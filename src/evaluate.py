...
def ffwd_video(path_in, path_out, checkpoint_dir, device_t=DEVICE, batch_size=BATCH_SIZE):
    ...
    with g.as_default(), g.device(device_t), tf.compat.v1.Session(
        config=soft_config
    ) as sess:
...
# get img_shape
def ffwd(data_in, paths_out, checkpoint_dir, device_t=DEVICE, batch_size=BATCH_SIZE):
    ...
    if len(remaining_in) > 0:
        ffwd(
            remaining_in, remaining_out, checkpoint_dir, device_t=device_t, batch_size=1
        )


def ffwd_to_img(in_path, out_path, checkpoint_dir, device=DEVICE):
    paths_in, paths_out = [in_path], [out_path]
    ffwd(paths_in, paths_out, checkpoint_dir, batch_size=1, device_t=device)


def ffwd_different_dimensions(
    in_path, out_path, checkpoint_dir, device_t=DEVICE, batch_size=BATCH_SIZE
):
